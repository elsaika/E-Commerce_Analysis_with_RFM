...
# ── FIND CSV ──────────────────────────────────────────────────────────────────
def _find_csv():
    """Cari main_data.csv di beberapa lokasi yang mungkin."""
    candidates = ["main_data.csv", "dashboard/main_data.csv"]
    # tambahkan path relatif terhadap file ini jika __file__ tersedia
    try:
...
def load_data(csv_path: str) -> pd.DataFrame:
    ...
    # Kolom turunan — buat jika belum ada di CSV
    ...
    if "shipping_ratio" not in df.columns:
...
# ── RFM — cache dengan path string agar hash-able ────────────────────────────
@st.cache_data
def compute_rfm(csv_path: str, year_filter: tuple) -> pd.DataFrame:
    """Hitung RFM dari CSV path + filter tahun (tuple agar bisa di-hash)."""
    df = pd.read_csv(csv_path, parse_dates=["order_purchase_timestamp"])
    df = df[df["order_purchase_timestamp"].dt.year.isin(year_filter)]
...
for col, title, value, sub in [
    (c1, "💰 Total Revenue",   f"R$ {total_rev/1e6:.2f}M",    "Semua transaksi delivered"),
    (c2, "🛍️ Total Orders",    f"{total_orders:,}",            "Order unik (status: delivered)"),
    (c3, "🧾 Avg Order Value", f"R$ {avg_order_val:,.0f}",     "Rata-rata per transaksi"),
    (c4, "👤 Total Pelanggan", f"{total_cust:,}",              "Pelanggan unik"),
]:
    with col:
        st.markdown(f"""<div class="metric-card">
            <h3>{title}</h3><h2>{value}</h2><small>{sub}</small>
...
with tab1:
    ...
    for i, val in enumerate(top_cat.values[::-1]):
        ax1.text(val + top_cat.max() * 0.01, i,
                 f"R$ {val/1e6:.2f}M", va="center", fontsize=7.5, color="#333")
    ax1.legend(
        handles=[mpatches.Patch(color=COLOR_RED,  label="Revenue tertinggi"),
                 mpatches.Patch(color=COLOR_BLUE, label="Kategori lainnya")],
    ...
    ax2 = axes[1]
    x = range(len(monthly))
    ax2.plot(x, monthly["payment_value"], color=COLOR_BLUE,
             linewidth=2.5, marker="o", markersize=5)
    ax2.fill_between(x, monthly["payment_value"], alpha=0.12, color=COLOR_BLUE)
    if len(monthly) > 1:
        peak_idx = int(monthly["payment_value"].idxmax())
        ax2.annotate(
...
with tab2:
    ...
    # Gunakan tuple agar cache bisa hash argumen
    rfm = compute_rfm(csv_path, tuple(sorted(sel_years)))
    ...
    ax1.legend(
        handles=[mpatches.Patch(facecolor=c, label=s) for s, c in zip(SEG_ORDER, SEG_COLORS)],
        fontsize=7.5, title="Segmen", title_fontsize=8, framealpha=0.7, loc="upper right"
    )

    ax2 = axes2[1]
    bars2 = ax2.bar(SEG_ORDER, avg_monetary.values, color=SEG_COLORS,
                    edgecolor="white", linewidth=0.8)
    ax2.set_title("Rata-rata Total Belanja (Monetary) per Segmen",
                  fontsize=11, fontweight="bold")
    ...
    annot_labels = rfm_pivot.map(lambda x: f"R${x:,.0f}" if not pd.isna(x) else "n/a")

    fig3, ax3 = plt.subplots(figsize=(10, 6))
    sns.heatmap(rfm_pivot, annot=annot_labels, fmt="", cmap="YlOrRd",
...
with tab3:
    ...
    with col4:
        ...
        if len(corr_cols) >= 2:
            ...
            ax.set_title("Korelasi Finansial & Fisik", loc="left",
                         fontsize=11, fontweight="bold")
            plt.tight_layout()
            st.pyplot(fig)
            plt.close(fig)
    ...
    n_rows = st.slider("Jumlah baris yang ditampilkan", 5, 50, 10, key="preview_slider")
